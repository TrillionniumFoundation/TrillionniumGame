...
SIGNER_SOURCE = r'''//! Bounded external-signer operation journal with verified epoch compaction.
//!
...
impl SigningOperationState {
    pub const fn is_terminal(self) -> bool {
        matches!(Self::Confirmed { .. } | Self::Rejected { .. }, self)
    }
}
...
impl fmt::Display for SignerJournalError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidCapacityRelationship => formatter.write_str(
                "signer journal active capacity cannot exceed epoch tombstone capacity",
            ),
            ...
            Self::CapacityExceeded { capacity } => {
                write!(formatter, "signer journal active set is full at capacity {capacity}")
            }
            ...
            Self::ActiveRecordsPreventEpochAdvance => formatter.write_str(
                "signer journal epoch cannot advance while active records remain",
            ),
...
fn validate_checkpoint_shape(
    ...
    let expected_epoch = checkpoint
        ...
        .get()
        .checked_add(1)
        .ok_or(SignerJournalError::CheckpointInvalid(
            "retired epoch overflow",
        ))?;
...
mod tests {
    ...
    fn response_loss_forbids_a_second_signing_dispatch() {
        ...
        assert_eq!(journal.reconcile(request, [7; 32], [8; 32]).unwrap(), confirmed);
    }
    ...
    fn checkpoint_restore_preserves_epoch_and_archive_chain() {
        ...
        journal.archive_terminal(request.handle(), [10; 32]).unwrap();
        let checkpoint = journal
            .advance_epoch(epoch(2), [12; 32], &verifier(12))
            .unwrap();
        let restored = SignerJournal::from_checkpoint(
            1,
            1,
            checkpoint.clone(),
            &verifier(12),
    ...
    fn archive_is_idempotent_only_for_exact_digest() {
        ...
        journal
            ...
            .unwrap();
        let archived = journal.archive_terminal(request.handle(), [10; 32]).unwrap();
        assert_eq!(
            journal.archive_terminal(request.handle(), [10; 32]).unwrap(),
            archived
        );
...
def main() -> None:
    (ROOT / "crates/trnm-token-crypto-provider/src/signer_journal.rs").write_text(
        SIGNER_SOURCE,
        encoding="utf-8",
    )
    ...
    replace_once(
        ...
        "};",
        "pub use signer_journal::{\n"
        "    SignerJournal, SignerJournalArchiveVerifier, SignerJournalCheckpoint,\n"
        "    SignerJournalEpoch, SignerJournalError, SignerReceiptBinding,\n"
        "    SigningOperationHandle, SigningOperationId, SigningOperationRecord,\n"
        "    SigningOperationState, SigningOperationTombstone, SigningRequest,\n"
        "};",
        "signer archive exports",
    )
