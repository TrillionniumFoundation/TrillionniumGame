...
def validate(source: str, contract: dict) -> None:
    for marker in REQUIRED:
        require(marker in source, f"remote provider missing {marker}")
    for marker in FORBIDDEN:
        require(marker not in source, f"remote provider contains forbidden marker {marker}")
    require(
        contract.get("schema") == "trillionnium.remote-mac-provider.v1",
        "contract schema",
    )
    require(
        contract.get("raw_key_bytes_enter_application_process") is False,
        "raw key boundary",
    )
    require(
        contract.get("local_software_fallback_allowed") is False,
        "fallback boundary",
    )
    ...
    require(contract.get("maximum_pending_connects") == 8, "pending connect bound")
...
def main() -> int:
    try:
        ...
        print(f"remote MAC provider validation failed: {error}", file=sys.stderr)
        return 1
    print("remote MAC provider boundary: OK")
    return 0
