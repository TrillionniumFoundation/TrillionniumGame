...
if [[ "$mode" == "--staged" ]]; then
  secret_scan_file="$tmpdir/staged-secret-scan"
  if ! python3 - "$root" >"$secret_scan_file" <<'PY'
import pathlib
import re
import subprocess
import sys
...
listed = subprocess.check_output(
    ["git", "-C", root, "diff", "--cached", "--name-only", "--diff-filter=ACM", "--no-renames", "-z"]
)
...
secret_assignment = re.compile(
    r'''(?mix)^\s*(?:export\s+)?["']?
    (TRNM_NAKAMA_AUTHORITY_PRIVATE_KEY|TRNM_NAKAMA_AUTHORITY_PRIVATE_KEYS|TRNM_HEPTA_ISSUER_PRIVATE_SEED|
    TRNM_AGENT_[A-Z0-9_]*PRIVATE_SEED|TRNM_NAKAMA_OPERATOR_TOKEN|
    TRNM_NAKAMA_DB_PASSWORD|NAKAMA_SERVER_KEY|NAKAMA_SOCKET_SERVER_KEY|
    NAKAMA_RUNTIME_HTTP_KEY|NAKAMA_CONSOLE_PASSWORD|NAKAMA_SESSION_ENCRYPTION_KEY|
    NAKAMA_SESSION_REFRESH_ENCRYPTION_KEY|NAKAMA_CONSOLE_SIGNING_KEY)
    ["']?\s*(?:=|:)\s*(?:"([^"\n]*)"|'([^'\n]*)'|([^\s#,\n]+))'''
)
...
for raw_path in listed.split(b"\0"):
    ...
    allowed_example = name.endswith((".example", ".sample", ".template"))
    sensitive_name = (
        name == ".env"
        or name.startswith(".env.")
        or name.endswith((".env", ".key", ".pem", ".seed", ".p12", ".pfx"))
        or name in {"id_rsa", "id_ed25519"}
    )
    if sensitive_name and not allowed_example:
        print(f"sensitive filename is staged: {path}")
        continue
    try:
        blob = subprocess.check_output(["git", "-C", root, "show", f":{path}"])
    except subprocess.CalledProcessError:
        continue
    if private_header.search(blob):
        print(f"private-key PEM material is staged: {path}")
    for pattern in token_patterns:
        if pattern.search(blob):
            print(f"high-confidence access token is staged: {path}")
            break
    if b"\0" in blob:
        continue
    text = blob.decode("utf-8", "replace")
    for match in secret_assignment.finditer(text):
        value = next(group for group in match.groups()[1:] if group is not None).strip().lower()
        placeholder = value.startswith("$") or any(
            marker in value for marker in ("replace-", "example", "fixture", "test-only")
        )
        if len(value) >= 16 and not placeholder:
            print(f"non-placeholder {match.group(1)} assignment is staged: {path}")
            break
PY
  then
    error "staged secret scanner failed; refusing a false-green commit"
  else
    while IFS= read -r finding; do
      [[ -n "$finding" ]] && error "$finding"
