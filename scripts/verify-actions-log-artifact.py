...
def request_json(token: str, url: str) -> dict[str, Any]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "trillionnium-outbox-log-verifier",
        },
    )
    try:
...
def archive_files(data: bytes) -> dict[str, bytes]:
    if not data or len(data) > MAX_ARCHIVE_BYTES:
        raise VerificationError(
            f"archive size is outside the bound: {len(data)} bytes"
        )
...
def verify_file_manifest(files: dict[str, bytes]) -> None:
    ...
    if set(expected) != actual_names:
        ...
        raise VerificationError(
            f"files.sha256 path set mismatch: missing={missing} extra={extra}"
        )
    ...
    *,
    ...
    require_env(
        identity,
        {
            ...
            "commit": head_sha,
            "profile": profile,
            "run_id": f"{run_id}-{run_attempt}-{profile}",
        },
        "identity.env",
    )
    ...
    if migration != expected_migration:
        raise VerificationError(
            f"identity.env: migration mismatch: {migration!r} != {expected_migration!r}"
        )
    ...
    require_env(
        result,
        {"status": "passed", "profile": profile, "commit": head_sha},
        "result.env",
    )
    ...
    return {
        "schema": "trillionnium.outbox-final-attempt-log-verification.v1",
        "repository": repository,
        "head_sha": head_sha,
        "run_id": run_id,
        ...
        "migration": migration,
        "migration_blob_sha1": identity["migration_blob_sha1"],
...
def main() -> int:
    ...
    try:
        ...
        if run.get("head_sha") != arguments.head_sha:
            raise VerificationError(
                f"run head mismatch: {run.get('head_sha')} != {arguments.head_sha}"
            )
        if str(run.get("run_attempt")) != arguments.run_attempt:
            raise VerificationError(
                f"run attempt mismatch: {run.get('run_attempt')} != {arguments.run_attempt}"
            )
        jobs_payload = request_json(
            token, f"{base}/actions/runs/{arguments.run_id}/jobs?per_page=100"
        )
        ...
        for profile in PROFILES:
            ...
            if len(matches) != 1:
                raise VerificationError(
                    f"expected one {expected_job_name!r} job, got {len(matches)}"
                )
            job = matches[0]
            if job.get("status") != "completed" or job.get("conclusion") != "success":
                raise VerificationError(
                    f"{expected_job_name} is not successful: "
            ...
            try:
                ...
                raise VerificationError(
                    f"{expected_job_name} log is not UTF-8"
                ) from error
            archive, envelope_metadata = EMITTER.parse(log_text)
            expected_name = (
                f"outbox-final-attempt-reaper-{profile}-{arguments.head_sha}-"
                f"{arguments.run_id}-{arguments.run_attempt}"
            )
            ...
            if record["archive_sha256"] != envelope_metadata.get("sha256"):
                raise VerificationError(
                    f"{expected_job_name} envelope/archive digest mismatch"
                )
            archive_path = arguments.output_directory / f"{profile}.tar.gz"
            record_path = arguments.output_directory / f"{profile}.json"
        ...
        summary = {
            "schema": "trillionnium.outbox-final-attempt-log-set.v1",
            "repository": arguments.repository,
            "head_sha": arguments.head_sha,
            "run_id": arguments.run_id,
            "run_attempt": arguments.run_attempt,
