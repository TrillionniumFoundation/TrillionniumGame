...
pub struct OutboxRecord {
    ...
    pub dead_reason: Option<Digest32>,
...
impl PgRepository {
    pub fn claim_outbox(
        ...
        while leases.len() < limit {
            ...
            let updated = transaction
                ...
                .map_err(map_postgres_error)?;
            require_one_fenced_update(updated, "outbox_claim_compare_and_swap_failed")?;
    ...
    pub fn complete_outbox(
        ...
        if receipt.is_zero() || completed_at_ms > lease.lease_expires_at_ms {
            return Err(invalid("invalid_outbox_completion"));
        }
        let generation = to_i64(lease.lease_generation)?;
        let attempt = i32::try_from(lease.attempt).map_err(|_| counter_overflow())?;
        let lease_expires_at_ms = to_i64(lease.lease_expires_at_ms)?;
        let completed_at_ms = to_i64(completed_at_ms)?;
        let updated = self
            .client
            .execute(
                "UPDATE trnm_outbox SET state = 2, owner_node = NULL, \
                 receipt_digest = $6, dead_reason_digest = NULL, updated_at_ms = $7 \
                 WHERE intent_id = $1 AND state = 1 AND owner_node = $2 \
                 AND lease_generation = $3 AND attempt = $4 AND available_at_ms = $5",
                &[
    ...
    pub fn retry_or_dead_letter_outbox(
        ...
        let lease_expires_at_ms_i64 = to_i64(lease.lease_expires_at_ms)?;
        let mut transaction = self
            .client
            .build_transaction()
            .isolation_level(postgres::IsolationLevel::Serializable)
            .start()
            .map_err(map_postgres_error)?;
        let attempt = load_fenced_attempt(
            &mut transaction,
            lease,
            generation,
            lease_expires_at_ms_i64,
            ...
            let updated = transaction
                .execute(
                    "UPDATE trnm_outbox SET state = 3, owner_node = NULL, \
                     receipt_digest = NULL, dead_reason_digest = $6, updated_at_ms = $7 \
                     WHERE intent_id = $1 AND state = 1 AND owner_node = $2 \
                     AND lease_generation = $3 AND attempt = $4 AND available_at_ms = $5",
                    &[
                        &lease.id.as_bytes().as_slice(),
    ...
    pub fn load_outbox_record(
        &mut self,
        intent_id: IntentId,
    ) -> Result<Option<OutboxRecord>, DomainError> {
        if intent_id.is_zero() {
            return Err(invalid("invalid_outbox_intent_id"));
        }
        let row = self
            .client
            .query_opt(
            ...
            .map_err(map_postgres_error)?;
        row.map(|row| decode_record(intent_id, &row)).transpose()
    }
}
    ...
    let row = transaction
        .query_opt(
            "SELECT attempt FROM trnm_outbox WHERE intent_id = $1 AND state = 1 \
             AND owner_node = $2 AND lease_generation = $3 AND available_at_ms = $4 FOR UPDATE",
            &[
                &lease.id.as_bytes().as_slice(),
                &lease.owner.as_bytes().as_slice(),
                &generation,
...
fn decode_record(intent_id: IntentId, row: &postgres::Row) -> Result<OutboxRecord, DomainError> {
    let state_value: i16 = row.get(0);
    let state = OutboxState::from_database_value(state_value)
        .ok_or_else(|| data_loss("invalid_outbox_state"))?;
    let attempt_i32: i32 = row.get(1);
    let attempt = u32::try_from(attempt_i32).map_err(|_| data_loss("invalid_outbox_attempt"))?;
    let lease_generation = from_i64(row.get(2), "negative_outbox_lease_generation")?;
    let owner = row
        .get::<_, Option<Vec<u8>>>(3)
        .map(|value| decode_id16::<NodeId>(value, NodeId::new, "invalid_outbox_owner_bytes"))
        .transpose()?;
    let receipt = row
        .get::<_, Option<Vec<u8>>>(4)
        .map(|value| decode_digest(value, "invalid_outbox_receipt_digest_bytes"))
        .transpose()?;
    let dead_reason = row
        .get::<_, Option<Vec<u8>>>(5)
        .map(|value| decode_digest(value, "invalid_outbox_dead_reason_digest_bytes"))
        .transpose()?;
    Ok(OutboxRecord {
        id: intent_id,
        state,
        ...
        receipt,
        dead_reason,
        available_at_ms: from_i64(row.get(6), "negative_outbox_available_at_ms")?,
        updated_at_ms: from_i64(row.get(7), "negative_outbox_updated_at_ms")?,
    })
}
...
mod tests {
    ...
    fn claim_limits_fail_closed() {
        let reason = Digest32::new([9; 32]);
        assert_eq!(
            validate_claim(NodeId::new([0; 16]), 1, 1, reason, 1)
                .unwrap_err()
                .reason(),
    ...
    #[test]
    fn lease_validation_fences_zero_generation_owner_and_expiry() {
        let mut value = lease();
