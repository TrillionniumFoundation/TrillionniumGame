...
use super::cancel::CancelTransport;
...
impl PgTlsConfig {
    ...
    fn connector(&self) -> Result<TlsConnector, DomainError> {
...
impl PgPool {
    ...
    pub fn connect_tls(
        ...
        let manager = PostgresConnectionManager::new(
            database,
            MakeTlsConnector::new(connector.clone()),
        );
        let pool = pool_builder(&policy)
            .build(manager)
    ...
    pub fn acquire(&self) -> Result<PgRepository, DomainError> {
        ...
        let (handle, cancel_transport) = match &self.inner {
            PoolInner::Plain(pool) => (
                pool.get_timeout(self.policy.acquire_timeout)
                    .map(ClientHandle::Plain),
                CancelTransport::Plain,
            ),
            PoolInner::Tls { pool, connector } => (
                pool.get_timeout(self.policy.acquire_timeout)
                    .map(ClientHandle::Tls),
                CancelTransport::Tls(connector.clone()),
            ),
        };
