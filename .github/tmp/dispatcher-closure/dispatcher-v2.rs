...
mod tests {
    ...
    use trnm_contracts::{CommandId, RetryClass};
    ...
    fn retryable_and_terminal_failures_use_distinct_attempt_ceiling() {
        for (failure, expected_max, expected_dead) in [
            (
                DeliveryFailure::retryable(Digest32::new([10; 32])),
                3,
                0,
            ),
            (
                DeliveryFailure::terminal(Digest32::new([11; 32])),
                1,
                1,
            ),
        ] {
