...
OUTPUT="$ROOT/docs/status/PLAN_V32_CONTROLLER_RUNS_V3.json"
LOG_DIR="$ROOT/docs/internal/plan-v32-controller-failures-v3"
...
workflows=(
  ...
  external-blocker-handoff-controller-20260909.yml
  stacked-database-pr-qualifier-v16-20260909.yml
)
work="$RUNNER_TEMP/plan-v32-controller-snapshot-v3"
rm -rf "$work"
mkdir -p "$work"
...
for workflow in "${workflows[@]}"; do
  ...
  if gh api "/repos/${GITHUB_REPOSITORY}/actions/workflows/${workflow}/runs?branch=${BRANCH}&per_page=1" \
    ...
    if [[ -n "$run_id" ]]; then
      ...
      if [[ "$conclusion" == failure || "$conclusion" == timed_out || "$conclusion" == cancelled || "$conclusion" == startup_failure ]]; then
...
python3 - "$work" "$OUTPUT" "$LOG_DIR" "$BRANCH" "${workflows[@]}" <<'PY'
...
work=Path(sys.argv[1]); output=Path(sys.argv[2]); log_dir=Path(sys.argv[3]); branch=sys.argv[4]; workflows=sys.argv[5:]
...
value={
    'schema':'trillionnium.plan-v32-controller-runs.v3',
...
for attempt in $(seq 1 12); do
  git commit -m 'status: snapshot complete Plan v3.2 controller runs' || :
