#!/usr/bin/env python3
"""Compose an independent CockroachDB semantic backup/restore evidence candidate."""
...
def load_postgresql_support() -> ModuleType:
    path = Path(__file__).with_name("compose_postgresql_semantic_recovery.py")
    spec = importlib.util.spec_from_file_location("postgresql_recovery_support", path)
    require(spec is not None and spec.loader is not None, "PostgreSQL recovery support unavailable")
...
def data_snapshot_sql(support: ModuleType) -> str:
    return "\n".join(
        line for line in support.data_snapshot_sql().splitlines()
...
def update_documents(root: Path) -> None:
    ...
    authority["cockroachdb_semantic_recovery_source_candidate"] = {
    ...
    boundary["cockroachdb_semantic_restore_source_candidate"] = True
    ...
    if "## CockroachDB semantic recovery candidate" not in text:
        text += section
...
def run(root: Path) -> None:
    ...
    support = load_postgresql_support()
    write(root / "scripts/cockroachdb-semantic-snapshot.sql", data_snapshot_sql(support))
    write(root / "scripts/check-cockroachdb-semantic-recovery.py", checker_source())
    write(root / "scripts/ci-cockroachdb-semantic-recovery.sh", harness_source())
    write(
        root / "tests/control_plane/test_cockroachdb_semantic_recovery_contract.py",
        test_source(),
    )
