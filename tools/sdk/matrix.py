...
def load_registry(path: Path) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != "trillionnium.sdk-source-snapshots.v1"
        or value.get("project_id") != "trillionnium-game"
    ):
    ...
    for row in profiles:
        ...
        if not row["repository"].startswith("heroiclabs/nakama-"):
            raise SDKMatrixError(
                "only reviewed Heroic Labs Nakama SDK repositories are allowed"
            )
...
def source_targets(
    nakama_root: Path, common_root: Path
    ...
    if len(rpcs) < 50 or len(messages) < 25:
        raise SDKMatrixError(
            f"implausible source denominator sizes: rpc={len(rpcs)} rt={len(messages)}"
        )
...
def scan_sdk(
    ...
    for path in sorted(root.rglob("*")):
        if (
            not path.is_file()
            or path.name == ".trillionnium-source-lock.json"
        ...
        for match in IDENTIFIER.finditer(text):
            # IDENTIFIER matches are ordered. Counting only the text between
            # consecutive matches makes line derivation linear in file size;
            # the previous implementation rescanned from byte zero for every
            # identifier and was quadratic on generated SDK sources.
...
def _matches(
    index: dict[str, list[dict[str, Any]]], target: str
) -> list[dict[str, Any]]:
    ...
    for key, locations in index.items():
        if (
            ...
            or (wanted.startswith(key) and len(key) >= 6)
        ):
            found.extend(locations)
    unique = {
        (item["path"], item["line"], item["identifier"]): item for item in found
    }
...
def generate(
    ...
    for profile in registry["profiles"]:
        ...
        for target in rpcs:
            leaves.append(
                make_leaf(profile, "api_operation", target, _matches(index, target))
            )
    ...
    manual = [
        {
            "class": "sdk_release_line_review",
            "symbol": "all-sdk-profiles",
            "reason": "Default-branch snapshots are discovery inputs, not approved release/support baselines.",
        },
        {
            "class": "sdk_transport_profile_review",
            "symbol": "all-sdk-profiles",
            "reason": "HTTP/JSON, WebSocket JSON/Protobuf, engine and platform transport behavior require black-box classification.",
        },
        {
            "class": "sdk_operation_match_review",
            "symbol": "all-sdk-profiles",
            "reason": "Identifier matches are candidates; generated aliases, overloads, serialization and error semantics require independent review.",
        },
        {
            "class": "sdk_repository_discovery_review",
            "symbol": "heroiclabs-nakama-sdk-set",
            "reason": "Archived, engine-specific and newly published official SDK repositories require explicit include/exclude decisions.",
        },
    ]
    value = {
