...
OUTPUT="$ROOT/docs/status/PLAN_V32_CONTROLLER_RUNS.json"
LOG_DIR="$ROOT/docs/internal/plan-v32-controller-failures"
...
workflows=(
  ...
  stacked-database-pr-qualifier-v12-20260909.yml
)
work="$RUNNER_TEMP/plan-v32-controller-snapshot"
rm -rf "$work"
mkdir -p "$work"
...
for workflow in workflows:
    ...
    row={'workflow':workflow,'run_id':None,'status':'absent','conclusion':None,'head_sha':None,'created_at':None,'updated_at':None,'jobs':[],'failure_log':None,'failure_log_sha256':None,'api_error':None}
    if runs_path.exists():
        value=json.loads(runs_path.read_text())
        runs=value.get('workflow_runs',[])
        if runs:
            run=runs[0]
            row.update(run_id=run.get('id'),status=run.get('status'),conclusion=run.get('conclusion'),head_sha=run.get('head_sha'),created_at=run.get('created_at'),updated_at=run.get('updated_at'))
            jobs_path=work/f'{key}.jobs.json'
            if jobs_path.exists():
                jobs=json.loads(jobs_path.read_text()).get('jobs',[])
                row['jobs']=[{'id':job.get('id'),'name':job.get('name'),'status':job.get('status'),'conclusion':job.get('conclusion')} for job in jobs]
...
summary={
    ...
    'failure':sum(row['status']=='completed' and row['conclusion'] not in {'success',None} for row in rows),
}
value={'schema':'trillionnium.plan-v32-controller-runs.v1','generated_at':dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(),'repository':'TrillionniumFoundation/TrillionniumGame','controller_branch':'codex/plan-v32-controller-v2-2026-09-09','summary':summary,'runs':rows,'claim_boundary':{'all_controllers_success':summary['success']==len(rows),'all_gaps_closed':False,'accepted_evidence':False,'production_ready':False}}
...
for attempt in $(seq 1 8); do
  git commit -m 'status: snapshot Plan v3.2 controller runs' || :
