...
class RequiredWorkflowRepositoryIntegrationTests(unittest.TestCase):
    ...
    def test_wave_two_requires_both_unit_and_live_execution_jobs(self):
        _, manifest = self.manifest()
        by_name = {entry.name: entry for entry in manifest.workflows}
        for name in ("pg-tls-rotation", "cockroach-serialization-retry"):
