...
class ActionsLogArtifactTests(unittest.TestCase):
    ...
    def test_binary_round_trip_preserves_size_and_sha256(self) -> None:
        data = bytes(range(256)) * 7 + b"\x00\xfftrillionnium"
        text, metadata = self.module.envelope(data, "evidence-archive.tar.gz", len(data))
    ...
    def test_missing_duplicate_and_corrupt_markers_fail_closed(self) -> None:
        ...
        with self.assertRaises(self.module.EnvelopeError):
            self.module.parse(text.replace("YXJjaGl2ZQ==", "YXJjaGl2ZQ=A"))
