...
from tools.denominator.common import REVISION, TREE, canonical_bytes
...
def fixture(root: Path) -> None:
    ...
    (root / ".trillionnium-source-lock.json").write_text(json.dumps({"repository": "heroiclabs/nakama", "revision": REVISION, "tree": TREE, "verification": "recomputed-git-tree-sha1"}), encoding="utf-8")
    write(root / "console/console.proto", APACHE + '''syntax = "proto3";
...
''')
    swagger = {"swagger": "2.0", "paths": {"/v2/console/user/{id}": {"get": {"operationId": "Console_GetUser", "parameters": [{"name": "id", "in": "path", "required": True}], "responses": {"200": {"schema": {"$ref": "#/definitions/nakama.console.User"}}}}}}, "definitions": {"nakama.console.User": {"type": "object", "required": ["id"], "properties": {"id": {"type": "string"}}}}}
    write(root / "console/console.swagger.json", json.dumps(swagger))
    write(root / "console/api.swagger.json", json.dumps({"swagger": "2.0", "paths": {}, "definitions": {}}))
    write(root / "console/acl/acl.go", "// Copyright Heroic Labs. All rights reserved. Proprietary; reproduction strictly forbidden.\npackage acl\n")
...
'''
    for relative in ("social/social.go", "server/api_authenticate.go", "server/api_link.go", "server/api_unlink.go"):
...
'''
    for relative in ("iap/iap.go", "iap/iap_samsung.go", "server/api_purchase.go", "server/api_subscription.go"):
...
class RemainingDenominatorTests(unittest.TestCase):
    def test_proto_and_swagger_extract_wire_contracts(self) -> None:
        items, manual = parse_proto('package x; service C { rpc Get (Req) returns (Res); } message Req { string id = 1; } enum E { X = 0; }')
        classes = {item["class"] for item in items}
        self.assertTrue({"console_grpc_service", "console_grpc_method", "console_proto_message", "console_proto_field", "console_proto_enum", "console_proto_enum_value"} <= classes)
        self.assertEqual(manual, [])
        parsed, manual = parse_swagger({"swagger": "2.0", "paths": {"/x": {"get": {"operationId": "C_Get", "responses": {}}}}, "definitions": {}}, "console")
    ...
    def test_restricted_acl_is_not_semantically_reproduced(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            restricted = [item for item in value["manual_contracts"] if item["class"] == "restricted_console_acl_source"]
    ...
    def test_provider_iap_and_ops_candidates_are_separate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(any(item["class"] == "provider_endpoint_candidate" for item in providers["leaves"]))
            self.assertTrue(any(item["class"] == "iap_database_statement_candidate" for item in iap["leaves"]))
            self.assertTrue(any(item["class"] == "metric_name_candidate" for item in metrics["leaves"]))
            self.assertTrue(any(item["class"] == "dockerfile_instruction" for item in ops["leaves"]))
