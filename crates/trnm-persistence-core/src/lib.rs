...
const OUTBOX_ATTEMPT_LIMIT_REASON: Digest32 = Digest32::new([
    0x88, 0x5b, 0xa9, 0x48, 0x7a, 0xba, 0xaa, 0x8d, 0x5d, 0xc1, 0xe5, 0x49, 0xbe, 0xae, 0x45,
    0x8a, 0xbd, 0x43, 0xbb, 0x3b, 0xb6, 0x64, 0xc8, 0x63, 0x7a, 0x40, 0x4f, 0x7c, 0xb3, 0x83,
    0xfe, 0xdb,
]);
...
mod tests {
    ...
    fn retry_limit_transitions_atomically_to_dead_letter() {
        ...
        assert_eq!(
            state.lease(intent_id, NodeId::new(id(2))).unwrap_err().reason(),
            "outbox_not_pending"
        );
    }
